...
- [ ] Decide what `reset` should mean, then implement a true reset that clears identity, credentials, and related persisted local state safely.
- [ ] Improve CLI output so operators can see whether the node is enrolled, approved, connected, or expired without reading source code.
- [ ] Add regression tests for `reset`, `info`, and `status`.

## Day 3 — Make debugging much easier
- [ ] Wire `src/utils/logger.ts` into bootstrap, approval, connection, execution, PTY, and file-operation flows.
- [ ] Ensure logs classify failures clearly: config, bootstrap, approval, credential, transport, exec, capability mismatch, and path violation.
- [ ] Add a documented log format example to the README or docs so operators know what they are looking at.
- [ ] Add tests around structured logging for the highest-value lifecycle events.
